...
{"time": true/false, "preference": true/false, "scene": true/false, "referenced_images": ["文件名"]}
...
- referenced_images 的判断（重要）：
  - 历史消息中可能出现旧格式【图片：描述】（全角冒号），那是没有文件名的旧记录，
    无法引用，一律不要列入 referenced_images
...
def _route_rule(user_msg: str) -> dict:
    ...
    return {"time": True, "preference": False, "scene": False, "referenced_images": []}
